...
namespace Menus.IntegrationTests.Fixtures;

public class MenusWebApplicationFixture : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public MenusWebApplicationFixture(WebApplicationFactory<Program> factory)
    {
        ...
        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                var descriptor = services.SingleOrDefault(
                    d => d.ServiceType == typeof(DbContextOptions<MenusDbContext>));
                ...
                services.AddScoped<IChangeTracker>(sp => sp.GetRequiredService<MenusDbContext>());
                services.AddScoped<IUnitOfWork>(sp => sp.GetRequiredService<MenusDbContext>());
                services.AddScoped<IQuery>(sp => sp.GetRequiredService<MenusDbContext>());
                services.AddScoped<IEntityLookup>(sp => sp.GetRequiredService<MenusDbContext>());
            });
        });
    ...
    public async Task<T> ExecuteWithDbContext<T>(Func<MenusDbContext, Task<T>> action)
    {
        using var scope = Services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<MenusDbContext>();
