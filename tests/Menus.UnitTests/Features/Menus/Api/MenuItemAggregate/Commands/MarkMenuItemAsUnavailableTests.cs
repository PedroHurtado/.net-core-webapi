...
public class MarkMenuItemAsUnavailableTests
{
    ...
    private readonly Mock<MarkMenuItemAsUnavailable.IRepository> _repositoryMock;
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly MarkMenuItemAsUnavailable.Service _service;

    public MarkMenuItemAsUnavailableTests()
    {
        _priceOptionCreate = new(_priceOptionValidator);
        _createMenuItem = new(_priceOptionCreate, _menuItemValidator);
        _markAsUnavailable = new(_menuItemValidator);
        _repositoryMock = new Mock<MarkMenuItemAsUnavailable.IRepository>();
        _unitOfWorkMock = new Mock<IUnitOfWork>();
        _service = new MarkMenuItemAsUnavailable.Service(_markAsUnavailable, _repositoryMock.Object, _unitOfWorkMock.Object);
    }
    ...
    [Fact]
    public async Task HandleAsync_WithMenuItem_ReturnsUnavailableResponse()
    {
    ...
    [Fact]
    public async Task HandleAsync_WithAvailableMenuItem_SetsCanBeOrderedFalse()
    {
    ...
    [Fact]
    public async Task HandleAsync_WithUnavailableMenuItem_RemainsUnavailable()
    {
        var menuItem = CreateMenuItem();
        _markAsUnavailable.Execute(menuItem, new MarkMenuItemAsUnavailableCommand());
        SetupRepositoryGet(menuItem);
    ...
    [Fact]
    public async Task HandleAsync_WithUnavailableMenuItem_DoesNotThrow()
    {
        var menuItem = CreateMenuItem();
        _markAsUnavailable.Execute(menuItem, new MarkMenuItemAsUnavailableCommand());
        SetupRepositoryGet(menuItem);
    ...
    public async Task Handler_WithValidId_ReturnsOkResult()
    {
        ...
        var result = await MarkMenuItemAsUnavailable.Handler(serviceMock.Object, menuItemId);

        result.Should().BeOfType<Ok<MenuItemResponse>>();
        var okResult = (Ok<MenuItemResponse>)result;
        okResult.Value.Should().Be(expectedResponse);
        okResult.Value!.IsAvailable.Should().BeFalse();
    }
    ...
    public async Task Handler_CallsServiceWithId()
    {
        ...
        await MarkMenuItemAsUnavailable.Handler(serviceMock.Object, menuItemId);
