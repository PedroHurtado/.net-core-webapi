...
public class ActivateProviderConfiguration : IFeatureModule
{
    ...
    {
        app.MapPost("/plans/{id}/provider-configurations/{provider}/activate", Handler);
    }
    ...
    public class Service(
        Plan.ActivateProviderConfiguration activateProviderConfig,
    ...
    ) : IService
    {
        public async Task<PlanResponse> HandleAsync(Guid id, string provider)
        {
            ...
            var command = new ActivateProviderConfigurationCommand(provider);
