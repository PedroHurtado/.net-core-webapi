...
public class DeactivatePlanPricingTier : IFeatureModule
{
    ...
    {
        app.MapPost("/plans/{id}/pricing-tiers/{billingPeriod}/deactivate", Handler)
            .RequirePlatform()
            .WithDescriptionCatalog("Deactivate a pricing tier");
    }
    ...
    public class Service(
        Plan.DeactivatePricingTier deactivatePricingTier,
    ...
    ) : IService
    {
        public async Task<PlanResponse> HandleAsync(Guid id, BillingPeriod billingPeriod)
        {
            ...
            var command = new DeactivatePricingTierCommand(billingPeriod);

            deactivatePricingTier.Execute(plan, command);
