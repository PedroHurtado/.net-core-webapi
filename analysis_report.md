...
### ✅ Tests Existentes (Corrección)
Los tests existen en `tests/Plan.UnitTests` y `tests/Plan.IntegrationTests`. Se debe verificar su cobertura y si coinciden con los casos de uso del dominio.
...
### ❌ Plan_Create.cs
*   **Error de Input**: El comando `CreatePlanCommand` recibe `Money Price`, `Features` y `ProviderConfigurations`.
    *   **Especificación (6.1)**: Debe recibir `Amount` y `CurrencyCode` (primitivos) y usar `Money.Create`. No debe recibir `Features` ni `ProviderConfigurations`.
*   **Error de Lógica**: Realiza lógica de adición de features y configs. Debe crear el plan limpio.

### ❌ Plan_Update.cs
*   **Error de Input**: El comando `UpdatePlanCommand` recibe `Money Price`.
    *   **Especificación (6.2)**: Debe recibir `Amount` y `CurrencyCode` (primitivos).
*   **Error de Lógica**: Asigna `Price` directamente. Debe usar `Money.Create` inyectado para crear el VO a partir de los primitivos.
