...
A partir de la especificación, genera los tests de integración de las slices y los helpers necesarios. Aplica la guía de estilo.

## Rutas de archivos

La ruta base es `src/$1.IntegrationTests/`. Dentro de ella:

- **Tests de slices command** → `Features/$1/Api/{Aggregate}Aggregate/Commands/{Action}{Aggregate}Tests.cs`
- **Tests de slices query** → `Features/$1/Api/{Aggregate}Aggregate/Queries/{Action}{Aggregate}Tests.cs`
- **Helpers** → `Helpers/{HelperName}.cs`
